  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === 'edit') {
        ...
        // API returns full updated record in res.data
        onSuccess?.({
          ...res.data,
          // if new image selected, show blob immediately;
          // otherwise res.data.image already has the server path
          _imageBlob: imageFile ? imagePreview : null,
        });
      } else {
        res = await createProduct(payload);
        handleApiMessage(res?.message);
        // API returns full created record in res.data
        onSuccess?.({
          ...res.data,
          _imageBlob: imagePreview, // blob URL for immediate preview
        });
      }
    } catch (err) {
      console.error(err);
